...
def register_post():
    ...
    if not email or not first_name or not last_name or not password1 or not password2:
        return render_template(
          'register.html',
          error_msg="Please provide your email, full name and password.",
        )
...
def ws_connect():
    ...
    CONNECTED_USERS[user.rowid] = ConnectedUser(user.rowid, user.first_name,user.last_name, request.sid)
